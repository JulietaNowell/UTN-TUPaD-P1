...
#EJERCICIO 6
#Funciones
def es_natural(mensaje, min = float("-Inf"), max = float("Inf")): 
...
def validar_numero_distinto_a_cero(mensaje, min = float("-Inf"), max = float("Inf")): 
    num = float(input(mensaje))
    while num == 0:
        num = float(input(f"ERROR! {mensaje}"))
    return num
...
print(f"Su IMC es: {imc:.2f}") # redondeo del IMC


#EJERCICIO 9
#Funciones
def validar_grados_Celsius(mensaje, min = float("-Inf"), max = float("Inf")): 
